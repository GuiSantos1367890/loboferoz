/**
 * @file Biblioteca de classes que representam os tipos mais comuns de <em>gráficos</em> a desenhar num <code>canvas</code> e também o próprio <code>canvas</code>, bem como os elementos <code>audio</code> e <code>video</code>. O propósito desta biblioteca é facilitar o desenvolvimento de jogos e outras aplicações multimédia num contexto académico, gerando-se algum nível de abstracção relativamente a alguns pormenores de implementação no código. Abordam-se apenas os tipos mais frequentes de gráficos e de elementos acessórios, evitando, quando possível, complexidade desnecessária.
 * @version 0.9.4
...
class Cronometro {
  ...
  /**
   * Construtor da classe <code>Cronometro</code>.
   */
  constructor() {
    this.#tempoInicial = Date.now();
    this.#tempoReferencia = Date.now();
